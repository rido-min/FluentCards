...
- For JSR, `scripts/build-jsr.mjs` rewrites the `.js` extensions in `src/` (needed by the npm CJS build) to `.ts` extensions (required by JSR) and writes the result to `jsr-src/` (gitignored)
- `jsr.json`'s `exports` points at `./jsr-src/index.ts`; the build script must run before any `deno check`, `deno publish`, or local `deno run` against `jsr-src/`
...
- Tests import from `../src/index.ts` (not `jsr-src/`) and use `--sloppy-imports` because the source uses `.js` extensions
...
- The build-jsr step runs before `deno publish` in CI; do the same locally before running `deno publish`
...
- Do NOT edit `jsr-src/` directly — it is regenerated by `scripts/build-jsr.mjs`
- JSR-specific config goes in `jsr.json` only

**Running Deno samples:**
```bash
cd node/samples/deno
deno run --sloppy-imports program.ts  # runs all samples
deno run --sloppy-imports basic_card_sample.ts  # run individual sample
```

**Import strategy in Deno samples:**
- Production: `import { ... } from 'jsr:@adaptivecards/fluent';`
- Local dev: `import { ... } from '../../packages/fluent-cards/src/index.ts';` (commented out by default; requires `--sloppy-imports`)
