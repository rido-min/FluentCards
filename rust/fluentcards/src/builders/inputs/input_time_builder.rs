...
/// Builds an Input.Time Adaptive Card element.
pub struct InputTimeBuilder {
    data: Card,
}

impl InputTimeBuilder {
    pub(crate) fn new() -> Self {
        let mut data = Card::new();
        data.insert("type".into(), Value::String("Input.Time".into()));
        data.insert("id".into(), Value::String(String::new()));
        InputTimeBuilder { data }
    }
